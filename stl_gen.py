...
class Model3D:
        ...
        for sphere in self.spheres:
            ...
            for tag in exclude_tags:
                if tag in sphere.tags:
                    ...
                    break
            if use:
                data.append(self._gen_sphere(sphere))
        for cylinder in self.cylinders:
            use = True
            for tag in exclude_tags:
                if tag in cylinder.tags:
                    ...
                    break
            if use:
                data.append(self._gen_cylinder(cylinder))
        if data:
            self.data = self._merge_shapes(data)

    def _gen_sphere(self, sphere):
    ...
    def _gen_cylinder(self, cylinder):
        ...
        for theta in np.linspace(0, 2*math.pi*(1 - 1/cylinder.granularity), cylinder.granularity):
            circle_pts[count,:] = radius*np.cos(theta) + np.cross(axis, radius)*np.sin(theta) + axis*np.dot(axis, radius)*(1 - np.cos(theta))
            count += 1
